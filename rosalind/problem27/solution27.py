...
def topological_ordering(graph):
    ...
    graph = set(graph)
    ordering = []
    candidates = list({edge[0] for edge in graph} -
                      {edge[1] for edge in graph})
...
def longest_path(graph, edges, source, sink):
    ...
    S = {node: -100 for node in {edge[0]
                                 for edge in graph.keys()} | {edge[1] for edge in graph.keys()}}
    S[source] = 0
    ...
    for node in top_order:
        try:
            S[node], backtrack[node] = max(map(lambda e: [S[e[0]] + graph[e], e[0]], filter(
                lambda e: e[1] == node, graph.keys())), key=lambda p: p[0])
...
def main():
    ...
    # Read the input data.
    with open('rosa.txt') as input_data:
        ...
        for pair in [line.strip().split('->') for line in input_data.readlines()]:
            ...
            edge_weight[int(pair[0]), int(pair[1].split(':')[0])
                        ] = int(pair[1].split(':')[1])
